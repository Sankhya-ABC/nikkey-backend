...
class SankhyaSyncOrdensServico extends Command
{
    protected $signature = 'sankhya:sync-os';
    protected $description = 'Busca ordens de servico no Sankhya e atualiza a base local (full sync).';

    public function handle(): int
    {
        $syncLog = (new SyncLogService('ordens-servico'))->start();
        try {
            $this->info('Iniciando sincronizacao total de Ordens de Servico...');
            ...
            foreach ($service->fetchPaginated(
                ...
                if (empty($records)) {
                    $this->warn("Pagina vazia ({$page['startRow']}). Continuando...");
                    continue;
                }
                ...
                $this->info("Pagina {$page['startRow']}--{$page['endRow']} processada ({$ordens->count()} OSs)");
            }

            $duracao = round(microtime(true) - $inicio, 2);
            $this->info("Total sincronizado: {$totalProcessadas} OSs em {$duracao}s.");

            $syncLog->finish($totalProcessadas);
            return 0;
        } catch (\Throwable $e) {
            $syncLog->fail($e);
