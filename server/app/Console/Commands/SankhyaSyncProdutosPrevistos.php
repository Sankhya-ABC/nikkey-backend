...
use App\Models\ProdutoPrevisto;
...
class SankhyaSyncProdutosPrevistos extends Command
{
    protected $signature = 'sankhya:sync-produtos-previstos';
    protected $description = 'Busca produtos previstos no Sankhya e atualiza a base local apenas se todas as FKs existirem.';

    public function handle(): int
    {
        $syncLog = (new SyncLogService('produtos-previstos'))->start();
        try {
            $this->info('Iniciando sincronizacao de produtos previstos...');
            ...
            if ($produtos->isNotEmpty()) {
                ProdutoPrevisto::upsert($produtos->toArray(), ['codprodprev_snk'], array_keys($produtos->first()));
            }

            $duracao = round(microtime(true) - $inicio, 2);
            $this->info("Total sincronizado: {$produtos->count()} produtos previstos em {$duracao}s.");
